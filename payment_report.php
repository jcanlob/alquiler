				...
				<div class="col-md-12">
					<form id="filter-report">
						<div class="row form-group">
							...
							<button class="btn btn-sm btn-block btn-primary col-md-2 ml-1">Aplicar Filtro</button>
						</div>
					</form>
					<hr>
						<div class="row">
						...
						<div class="row">
							<table class="table table-bordered">
								...
								<tbody>
									...
									$i = 1;
									$tamount = 0;
									$payments  = $conn->query("SELECT p.*,concat(t.lastname,', ',t.firstname) as name,h.house_no FROM payments p inner join tenants t on t.id = p.tenant_id inner join houses h on h.id = t.house_id where date_format(p.date_created,'%Y-%m') = '$month_of' order by unix_timestamp(date_created)  asc");
								...
								<?php else: ?>
									<tr>
										<th colspan="6"><center>No se encontraron datos.</center></th>
									</tr>
								<?php endif; ?>
								</tbody>
								<tfoot>
									<tr>
										<th colspan="5">Cantidad Total</th>
