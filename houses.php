				...
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								<label class="control-label">Propiedad Nº</label>
							...
							<div class="form-group">
								...
								<?php endwhile; ?>
								<?php else: ?>
									<option selected="" value="" disabled="">Por favor consulte la lista de categorías.</option>
								<?php endif; ?>
								</select>
							</div>
							<div class="form-group">
						...
						<table class="table table-bordered table-hover">
							...
							<tbody>
								...
								<tr>
									...
									<td class="">
										<p>Propiedad #: <b><?php echo $row['house_no'] ?></b></p>
										<p><small>Tipo Propiedad: <b><?php echo $row['cname'] ?></b></small></p>
									...
									<td class="text-center">
										...
										<button class="btn btn-sm btn-danger delete_house" type="button" data-id="<?php echo $row['id'] ?>">Borrar</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					...
					$('#msg').html('<div class="alert alert-danger">El número de casa ya existe.</div>')
					end_load()
				}
			}
		})
	...
	$('.delete_house').click(function(){
		_conf("¿Esta seguro de borrar esta propiedad?","delete_house",[$(this).attr('data-id')])
	})
	function delete_house($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Propiedad borrada",'success')
					setTimeout(function(){
						location.reload()
					},1500)
